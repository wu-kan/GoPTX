...
FROM nvidia/cuda:12.6.2-devel-ubuntu24.04 as devel
...
apt-get install --no-install-recommends -y \
  wget cmake clang-18 ninja-build \
...
CC=clang-18 CXX=clang++-18 CUDAHOSTCXX=clang++-18 CUDACXX=nvcc CUDAARCHS="80;90" /root/GoPTX/test/utils/build.sh /root/GoPTX_nvcc
...
cat d.ptx
EOF
FROM nvidia/cuda:12.6.2-base-ubuntu24.04 as base
ENV PATH=/root/GoPTX_nvcc/bin:$PATH \
  CPATH=/root/GoPTX_nvcc/include:$CPATH \
  LIBRARY_PATH=/root/GoPTX_nvcc/lib:$LIBRARY_PATH \
  LD_LIBRARY_PATH=/root/GoPTX_nvcc/lib:$LD_LIBRARY_PATH
